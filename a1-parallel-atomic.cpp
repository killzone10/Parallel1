...
public:
    ...
    void notify(){ // notify so producer can do it from outside
        cond.notify_one();
    ...
    std::shared_ptr<T> wait_and_pop(T &value) // not used
    {
        // todo: 
...
int producer(std::string filename, SafeQ<int> &q)
{
    ...
    while (!ifs.eof()) {
        ...
        q.push(num);
        i++;
        if (i % 50 == 0){ // every nth iteration notify - it resolves boottleneck
            q.notify();
        }
        produced_count++;
    }
...
struct WorkerArgs { // its better to have everything in struct
    std::atomic<int> primes = 0;
    std::atomic<int> nonprimes = 0;
    std::atomic<double> sum = 0.0;
    std::atomic<int> consumed_count = 0;
    std::vector<std::atomic<int>> number_counts;
    WorkerArgs() : number_counts(10) {
        for (auto& b : number_counts) { std::atomic_init(&b, 0); }
    }
};
...
void worker(SafeQ<int> &q,  WorkerArgs& args, int id)
{
    ...
    for (int i = 0; i < 10; i++) {
        args.number_counts.at(i) += number_counts[i]; // at() was used because vector is not so good if it comes to atomic variables
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num_threads; ++i) {
        q.push(END_MARKER);
        q.notify(); // notifying so there is no deadlock
