 ...
 *
 */
void print_output(int num_threads, int primes, int nonprimes, double mean, std::vector<std::atomic<int>> &number_counts, std::chrono::high_resolution_clock::time_point t1, std::chrono::high_resolution_clock::time_point t2, bool only_exec_times, bool no_exec_times)
{
    if (!only_exec_times)
    {
        ...
        for (int j = 0; j < 10; ++j)
        {
            std::cout << j + 1 << ": " << number_counts.at(j) << std::endl;
        }
